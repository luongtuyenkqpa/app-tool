...
--==================================================
-- LOGIC AUTO NÉ v4 — Dual Detection: HP Poll + Touched Hook
-- Phương pháp 1: Poll HP mỗi frame → khi HP tụt → né ngay (hoạt động với MỌI game)
-- Phương pháp 2: Hook Touched weapon địch (backup thêm)
--==================================================
local neConns       = {}
local autoNeConn    = nil   -- Heartbeat HP-poll
local lastDodgeTime = 0
local DODGE_COOLDOWN = 0.6  -- giảm cooldown để né nhạy hơn
local neTrackedHP   = 0     -- HP frame trước để detect bị đánh

-- Tìm enemy gần nhất (để tính hướng né)
...
-- Né bằng cách dịch chuyển CFrame + LinearVelocity
local function doDodge(enemyRoot)
    ...
    -- Ưu tiên enemy root truyền vào, nếu không có thì tìm nearest
    ...
    if eRoot then
        ...
        toMe = (toMe.Magnitude > 0.1) and toMe.Unit or Vector3.new(1, 0, 0)
        local side = toMe:Cross(Vector3.new(0, 1, 0)).Unit
        local sign = (math.random(0, 1) == 0) and 1 or -1
        dodgeOffset = (side * sign * 16) + (toMe * 10)
    else
        local angle = math.random() * math.pi * 2
        dodgeOffset = Vector3.new(math.cos(angle) * 16, 0, math.sin(angle) * 16)
    end
    ...
    -- LinearVelocity (thay BodyVelocity deprecated)
    pcall(function()
        local att = Instance.new("Attachment", myRoot)
        local lv  = Instance.new("LinearVelocity", myRoot)
        lv.Attachment0        = att
        lv.VectorVelocity     = dodgeOffset.Unit * 35 + Vector3.new(0, 12, 0)
        lv.MaxForce           = 9e4
        lv.RelativeTo         = Enum.ActuatorRelativeTo.World
        game:GetService("Debris"):AddItem(lv,  0.13)
        game:GetService("Debris"):AddItem(att, 0.15)
    end)

    -- Fallback BodyVelocity nếu game cũ
    pcall(function()
        local bv = Instance.new("BodyVelocity")
        bv.Velocity  = dodgeOffset.Unit * 35 + Vector3.new(0, 12, 0)
        bv.MaxForce  = Vector3.new(9e4, 9e4, 9e4)
        bv.P         = 9e4
        bv.Parent    = myRoot
        game:GetService("Debris"):AddItem(bv, 0.13)
    end)

    -- Update trackedHP để tránh né 2 lần liên tiếp vì HP chưa cập nhật
    if myHum then neTrackedHP = myHum.Health end
end

-- Hook Touched weapon địch (backup method)
...
local function hookEnemyWeapons(enemyPlayer)
    ...
    for _, obj in ipairs(char:GetChildren()) do
        if obj:IsA("Tool") then
            for _, part in ipairs(obj:GetDescendants()) do
                if part:IsA("BasePart") then hookWeaponPart(part, eRoot) end
            end
        end
    end
    local c = char.ChildAdded:Connect(function(obj)
        ...
        if obj:IsA("Tool") then
            ...
            for _, part in ipairs(obj:GetDescendants()) do
                if part:IsA("BasePart") then hookWeaponPart(part, eR) end
            end
...
local function startAutoNe()
    ...
    -- PHƯƠNG PHÁP CHÍNH: Poll HP mỗi Heartbeat
    -- Khi HP giảm đột ngột → tức là bị đánh → né ngay (không phụ thuộc vào Touched)
    autoNeConn = RunService.Heartbeat:Connect(function()
        ...
        -- Phát hiện bị đánh: HP tụt >= 1 điểm so với frame trước
        if curHP < neTrackedHP - 1 then
            doDodge(nil)  -- né về hướng nearest enemy
        end
        neTrackedHP = curHP
    end)

    -- PHƯƠNG PHÁP PHỤ: Hook Touched weapon địch
    for _, p in ipairs(Players:GetPlayers()) do
...
--==================================================
-- LOGIC TĂNG SÁT THƯƠNG v4
-- Dùng hitbox Part vô hình gắn vào weapon của mình
-- Khi Touched địch → TakeDamage trực tiếp (hoạt động với NPC/Dummy và player nếu server cho phép)
-- + Scan RemoteEvent để FireServer với hitbox data
--==================================================
local dmgBoostConns = {}
local dmgBoostConn  = nil
local dmgHbConn     = nil  -- Heartbeat proximity damage

-- Scan tất cả RemoteEvent/Function có tên liên quan đến hit/damage
    ...
    local function scan(obj)
        for _, v in ipairs(obj:GetChildren()) do
            if v:IsA("RemoteEvent") or v:IsA("RemoteFunction") or v:IsA("BindableEvent") then
                ...
                or n:find("deal") or n:find("hurt") or n:find("wound") then
...
-- Tạo hitbox vô hình gắn vào weapon part để trigger damage
local function createBoostHitbox(weaponPart)
    local hb = Instance.new("Part")
    hb.Name          = "SDST_BoostHitbox"
    hb.Size          = weaponPart.Size + Vector3.new(0.4, 0.4, 0.4)
    hb.Transparency  = 1
    hb.CanCollide    = false
    hb.Anchored      = false
    hb.Massless      = true
    ...
    weld.Parent = hb
    hb.CFrame = weaponPart.CFrame
    hb.Parent = weaponPart.Parent
    ...
    local c = hb.Touched:Connect(function(hit)
        ...
        if victimChar == LocalPlayer.Character then return end
        local victimHum = victimChar:FindFirstChildOfClass("Humanoid")
        if not victimHum or victimHum.Health <= 0 then return end

        -- Cooldown per victim để không spam quá nhiều
        local vid = tostring(victimChar)
        local now = tick()
        if lastHitTime[vid] and (now - lastHitTime[vid]) < 0.25 then return end
        lastHitTime[vid] = now

        -- Extra damage dựa trên MaxHealth để scale hợp lý với mọi game
        local extra = victimHum.MaxHealth * (STATE.boostLevel / 100) * 0.20
        extra = math.max(extra, 5)
        pcall(function() victimHum:TakeDamage(extra) end)

        -- Thử FireServer các remote hit tìm được (pattern phổ biến Dueling Games)
        pcall(function()
            local remotes = findDamageRemotes()
            for _, rem in ipairs(remotes) do
                if rem:IsA("RemoteEvent") then
                    pcall(function()
                        rem:FireServer(victimChar, extra, weaponPart.CFrame.Position)
                    end)
                    pcall(function()
                        rem:FireServer(victimHum, extra)
                    end)
                    pcall(function()
                        rem:FireServer(victimChar)
                    end)
                elseif rem:IsA("BindableEvent") then
                    pcall(function() rem:Fire(victimChar, extra) end)
                end
            end
        end)
    end)
    table.insert(dmgBoostConns, c)
    table.insert(dmgBoostConns, {
        Disconnect = function()
...
local function hookMyWeapon(tool)
    for _, part in ipairs(tool:GetDescendants()) do
        if part:IsA("BasePart") and part.Name ~= "SDST_BoostHitbox" then
            -- Method 1: Touched trực tiếp trên part gốc
            local c = part.Touched:Connect(function(hit)
                if not STATE.damageBoost then return end
                local victimChar = hit:FindFirstAncestorOfClass("Model")
                if not victimChar or victimChar == LocalPlayer.Character then return end
                local victimHum = victimChar:FindFirstChildOfClass("Humanoid")
                if not victimHum or victimHum.Health <= 0 then return end
                local extra = victimHum.MaxHealth * (STATE.boostLevel / 100) * 0.20
                extra = math.max(extra, 5)
            ...
            -- Method 2: Hitbox vô hình bọc ngoài weapon
            pcall(function() createBoostHitbox(part) end)
        end
    end

    local c2 = tool.DescendantAdded:Connect(function(desc)
        if desc:IsA("BasePart") and desc.Name ~= "SDST_BoostHitbox" and STATE.damageBoost then
            task.wait(0.05)
...
local function startDamageBoost()
    ...
    if dmgHbConn    then dmgHbConn:Disconnect()    dmgHbConn    = nil end

    -- Hook weapon hiện tại
    local myChar = LocalPlayer.Character
    if myChar then
    ...
    -- PHƯƠNG PHÁP PHỤ: Proximity damage mỗi 0.3s khi đang swing
    -- (Heartbeat check nếu đang cầm weapon và có địch trong tầm ngắn)
    local swingCooldown = {}
    dmgHbConn = RunService.Heartbeat:Connect(function()
        if not STATE.damageBoost then return end
        local char = LocalPlayer.Character
        if not char then return end
        -- Chỉ kích hoạt nếu đang cầm tool
        local hasTool = false
        for _, obj in ipairs(char:GetChildren()) do
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character then
                ...
                if eRoot and eHum and eHum.Health > 0 then
                    ...
                    -- Chỉ trong tầm cận chiến ~6 studs
                    if dist < 6 then
                        local pid = tostring(p.UserId)
                        if not swingCooldown[pid] or (now - swingCooldown[pid]) > 0.35 then
                            swingCooldown[pid] = now
                            local extra = eHum.MaxHealth * (STATE.boostLevel / 100) * 0.08
...
local function stopDamageBoost()
    ...
    if myChar then
        for _, d in ipairs(myChar:GetDescendants()) do
            if d.Name == "SDST_BoostHitbox" then pcall(function() d:Destroy() end) end
        end
    end
end

--==================================================
-- LOGIC GIẢM SÁT THƯƠNG NHẬN v4
-- Chiến lược: HealthChanged signal + Heartbeat heal-back loop
-- Khi HP tụt → spawn coroutine bù máu liên tục qua Humanoid:TakeDamage âm (Heal)
-- Đây là cách duy nhất hoạt động trên client Roblox FilteringEnabled
--==================================================
local reductConn   = nil
local reductHbConn = nil
local reductHealConn = nil
...
local function applyDamageReduction(on)
    ...
    if reductHealConn then reductHealConn:Disconnect() reductHealConn = nil end
    ...
    local healBudget  = 0      -- lượng máu còn cần bù
    local HEAL_RATE   = 0.05   -- bù máu chia nhỏ mỗi 0.05s để mượt

    -- PHƯƠNG PHÁP 1: HealthChanged signal (chính xác, nhạy hơn PropertyChanged)
    -- Roblox cung cấp HealthChanged với giá trị HP mới
    reductConn = myHum.HealthChanged:Connect(function(newHP)
        if not STATE.damageReduct then return end
        if newHP <= 0 then prevHP = 0 healBudget = 0 return end

        if newHP < prevHP - 0.5 then
            -- Bị mất máu → tính refund
            local lost   = prevHP - newHP
            local refund = lost * reductFrac
            healBudget   = healBudget + refund  -- cộng dồn vào budget
        end
        prevHP = newHP
    end)

    -- PHƯƠNG PHÁP 2: Heartbeat heal loop — thực hiện bù máu từng tick nhỏ
    -- Dùng Humanoid:TakeDamage với giá trị âm HOẶC set Health trực tiếp
    reductHbConn = RunService.Heartbeat:Connect(function(dt)
        if not STATE.damageReduct then return end
        if healBudget <= 0 then return end

        local curHum = myChar and myChar:FindFirstChildOfClass("Humanoid")
        if not curHum or curHum.Health <= 0 then healBudget = 0 return end

        -- Tính lượng heal frame này (tối đa 5 HP mỗi frame để mượt)
        local healNow = math.min(healBudget, math.max(1, healBudget * (dt / HEAL_RATE)))
        healNow = math.min(healNow, curHum.MaxHealth - curHum.Health)

        if healNow > 0.05 then
            -- Thử set Health trực tiếp (hoạt động nếu server không block)
            local ok = pcall(function()
                curHum.Health = math.min(curHum.MaxHealth, curHum.Health + healNow)
            end)

            -- Nếu set trực tiếp thất bại, thử Heal method
            if not ok then
                pcall(function()
                    curHum:TakeDamage(-healNow)
                end)
            end
    ...
    -- PHƯƠNG PHÁP 3: PropertyChanged backup cho game cũ
    reductHealConn = myHum:GetPropertyChangedSignal("Health"):Connect(function()
        if not STATE.damageReduct then return end
        ...
        -- không update prevHP ở đây (để HealthChanged xử lý chính)
    end)
end
