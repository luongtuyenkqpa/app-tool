...
-- ══════════════════════════════════════════════════════════
--  LIGHTNING DETECTION ENGINE  (v2 — Smart, no-spam)
--
--  Logic:
--  1. Chỉ chạy khi người chơi ĐANG có cây trồng (myActiveTrees ~= empty)
--  2. Cooldown GLOBAL 5 giây — chỉ 1 thông báo duy nhất mỗi 5s
--  3. Cây được theo dõi qua event: detect khi cây MỚI được thêm vào workspace
--  4. Khi cây bị xóa → xóa khỏi danh sách theo dõi
-- ══════════════════════════════════════════════════════════

local myPlot        = nil   -- plot của người chơi
-- myActiveTrees: {[obj] = {pos, name}} — chỉ cây đang trồng hiện tại
local myActiveTrees = {}
local hasTrees      = false  -- true khi đang có cây

-- Cooldown toàn cục: chỉ 1 thông báo / 5 giây
local ALERT_COOLDOWN   = 5      -- giây
local lastAlertTime    = -999   -- tick() của lần cảnh báo cuối
local alertInCooldown  = false

-- ── Hàm phát cảnh báo DUY NHẤT ───────────────────────────
local function fireAlert(playerName, treeName)
    local now = tick()
    -- Kiểm tra cooldown global
    if alertInCooldown or (now - lastAlertTime) < ALERT_COOLDOWN then
        return
    end
    lastAlertTime   = now
    ...
    -- 2) Thông báo nổi (chỉ 1 lần)
    notify(
        "⚡  SET SAP DANH!",
        playerName .. "\nCay " .. treeName ..
        " bi set trong " .. STATE.alertTime .. " giay!\nHay thoat khoi khu vuc!",
        "lightning",
        5
    )
    ...
    task.delay(ALERT_COOLDOWN, function()
...
local function findMyPlot()
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        ...
        if owner == lp.Name then
            myPlot = obj.Parent or obj; return myPlot
        end
    end
    return nil
end

-- ── Kiểm tra tên có phải cây không ───────────────────────
local function isTreeName(name)
    local n = name:lower()
    return n:find("tree") or n:find("plant") or n:find("crop")
        or n:find("seed") or n:find("grow") or n:find("cay")
        or n:find("fruit") or n:find("mount") or n:find("trong")
end
...
local function isLightningObject(obj)
    ...
    or n:find("strike") or n:find("electric") or n:find("zap")
    or n:find("spark") or n:find("thunderstruck") then
        return true
    end
    if obj:IsA("BasePart") then
        local col = obj.Color
        -- Màu vàng rực (R cao, G vừa, B thấp) + Neon = dấu hiệu sét
        if col.R > 0.85 and col.G > 0.72 and col.B < 0.25
        and obj.Material == Enum.Material.Neon then
            return true
        end
    end
    return false
...
-- ── Lấy vị trí của cây (BasePart hoặc Model) ─────────────
local function getTreePos(obj)
    if obj:IsA("Model") then
        if obj.PrimaryPart then return obj.PrimaryPart.Position end
        -- fallback: tìm part đầu tiên
...
-- ── Đăng ký theo dõi 1 cây ───────────────────────────────
local function registerTree(obj)
    if myActiveTrees[obj] then return end  -- đã đăng ký rồi
    local pos = getTreePos(obj)
    if not pos then return end
    myActiveTrees[obj] = {pos = pos, name = obj.Name}
    hasTrees = true
    print("[LightningDetector] Theo doi cay moi:", obj.Name, "tai", tostring(pos))

    -- Tự động hủy đăng ký khi cây bị xóa
    obj.AncestryChanged:Connect(function()
        if not obj.Parent then
            myActiveTrees[obj] = nil
            -- Kiểm tra còn cây nào không
            local count = 0
            for _ in pairs(myActiveTrees) do count = count + 1 end
            hasTrees = count > 0
            if not hasTrees then
                print("[LightningDetector] Khong con cay nao - tam dung theo doi")
            end
        end
    end)
end

-- ── Scan cây hiện có trong plot ───────────────────────────
local function scanExistingTrees()
    local plot = myPlot or findMyPlot()
    local hrp  = char and char:FindFirstChild("HumanoidRootPart")

    if plot then
        for _, obj in ipairs(plot:GetDescendants()) do
            if (obj:IsA("BasePart") or obj:IsA("Model")) and isTreeName(obj.Name) then
                registerTree(obj)
            end
        end
    elseif hrp then
        -- Fallback: cây trong bán kính 70 studs quanh nhân vật
        for _, obj in ipairs(workspace:GetDescendants()) do
            if obj:IsA("BasePart") and isTreeName(obj.Name) then
                local d = (obj.Position - hrp.Position).Magnitude
                if d < 70 then registerTree(obj) end
            end
        end
    end
end

-- ── Kiểm tra sét gần cây nào đó của mình ─────────────────
local function checkLightningNearTrees(objPos)
    if not hasTrees then return end
    for treeObj, treeInfo in pairs(myActiveTrees) do
        -- Cập nhật vị trí cây (có thể di chuyển nếu game animate)
        local freshPos = getTreePos(treeObj)
        if freshPos then treeInfo.pos = freshPos end

        local dist = (objPos - treeInfo.pos).Magnitude
        if dist < STATE.alertRadius then
            -- Kích hoạt cảnh báo (có cooldown bên trong)
            fireAlert(lp.DisplayName, treeInfo.name)
            return  -- chỉ cần báo 1 lần dù nhiều cây bị đe dọa
        end
    end
end

-- ══════════════════════════════════════════════════════════
--  HOOK 1: Phát hiện cây MỚI được trồng
...
workspace.DescendantAdded:Connect(function(obj)
    ...
    -- Cây mới được trồng?
    if (obj:IsA("BasePart") or obj:IsA("Model")) and isTreeName(obj.Name) then
        -- Xác nhận cây gần plot/nhân vật của mình
        task.delay(0.1, function()
            ...
            local plot = myPlot or findMyPlot()
            local isMyTree = false

            if plot and obj:IsDescendantOf(plot) then
                isMyTree = true
            elseif hrp and (pos - hrp.Position).Magnitude < 80 then
                isMyTree = true
            end
    ...
    if isLightningObject(obj) then
        task.delay(0.05, function()
            ...
            checkLightningNearTrees(objPos)
        end)
    end
end)

-- ══════════════════════════════════════════════════════════
--  HOOK 2: Polling nhẹ (mỗi 0.5s thay vì 0.15s)
--  Chỉ chạy khi đang có cây — scan sét đã tồn tại trong workspace
-- ══════════════════════════════════════════════════════════
task.spawn(function()
    -- Scan cây có sẵn ngay khi khởi động
    task.wait(1)
    scanExistingTrees()

    while task.wait(0.5) do
        if not STATE.lightningAlert then continue end
        if not hasTrees then continue end  -- KHÔNG check nếu không có cây

        -- Scan sét hiện có trong workspace
        for _, obj in ipairs(workspace:GetDescendants()) do
            if not isLightningObject(obj) then continue end
            if not obj:IsA("BasePart") then continue end

            checkLightningNearTrees(obj.Position)
            -- Dừng sớm nếu đã vào cooldown (tránh scan thêm sau khi đã báo)
            if alertInCooldown then break end
        end
    end
end)

-- ── Rescan khi character respawn ──────────────────────────
lp.CharacterAdded:Connect(function(newChar)
    char = newChar
    task.wait(2)
    -- Reset và scan lại cây
    myActiveTrees = {}
    hasTrees      = false
    myPlot        = nil
    scanExistingTrees()
