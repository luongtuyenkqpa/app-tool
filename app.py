                ...
                for update in res.get("result", []):
                    ...
                    if "message" in update:
                        ...
                        if text.startswith("/start"):
                            ...
                            welcome = f"🌟 <b>HỆ THỐNG CẤP PROXY TỰ ĐỘNG</b> 🌟\n\nXin chào <b>{user_first_name}</b>!\nNhấn vào nút bên dưới để Đăng ký/Đăng nhập mua Key và Lấy Proxy:"
...
# [MỚI] Thuật toán sinh Key Proxy 15 ký tự chữ thường
...
# ========================================================
# TẢI CHỨNG CHỈ CA CHO PROXY
# ========================================================
@app.route('/download-ca')
def download_ca():
    # Trả về nội dung giả lập hoặc file thực tế của MITM CA
...
def tg_activate_proxy():
    ...
    with db_lock:
        ...
        if not kd.get("bound_olm"):
            return jsonify({"status": "error", "msg": "Vui lòng yêu cầu Admin chỉ định tài khoản OLM trước khi Kích hoạt Proxy!"})
...
# ========================================================
# API TELEGRAM MINI APP ADMIN (C-PANEL)
# ========================================================
...
def tg_admin_setup_proxy():
    ...
    with db_lock:
        ...
        if not db["keys"][key].get("bound_olm"):
            return jsonify({"status": "error", "msg": "Key này chưa được chỉ định tài khoản OLM. Phải ghim OLM trước khi cài Proxy!"})
    ...
    return jsonify({"status": "success", "msg": f"Thiết lập thành công!\nMáy chủ: {host}\nCổng: {rand_port}"})
...
def telegram_mini_app():
    ...
    <head>
        ...
        <title>Hệ Thống LVT Proxy App</title>
    ...
    <body>
        ...
        <div id="scr-dash" class="screen">
            ...
            <div id="tab-act" style="display:block;">
                <div class="card border-0" style="background:rgba(0,255,204,0.03);">
                    <p style="font-size:12px; color:#8892b0; margin-top:0;">Nhập Key kích hoạt để lấy cấu hình Proxy OLM.</p>
                ...
                <div class="card" id="proxy-result" style="display:none; border-color:#bd00ff;">
                    <h4 style="color:#bd00ff; margin-top:0;"><i class="fas fa-network-wired"></i> THÔNG TIN PROXY</h4>
                    <p style="font-size:13px; color:#ccc;">Tên máy chủ: <strong id="res-host" style="color:#00ffcc;"></strong></p>
                    <p style="font-size:13px; color:#ccc;">Cổng: <strong id="res-port" style="color:#00ffcc;"></strong></p>
                    <button class="btn-neon mt-3" style="background:linear-gradient(90deg, #a855f7, #6366f1);" onclick="window.location.href='/download-ca'"><i class="fas fa-download"></i> TẢI CHỨNG CHỈ (CA)</button>
                </div>
            </div>
        ...
        <div id="screen-admin-main" class="screen">
            <div class="header-bar">
                ...
                <div style="background:#ffcc00; color:#000; padding:6px 12px; border-radius:10px; font-weight:900; font-size:12px;">ADMIN</div>
            </div>
            
            <div class="select-btn" onclick="navTo('screen-admin-keys')">
                <div class="select-btn-left d-flex align-items-center gap-3">
                    <div class="icon-box" style="background:rgba(34, 197, 94, 0.15); color:#22c55e;"><i class="fas fa-magic"></i></div>
                    <div><div style="font-size:15px; font-weight:800; color:#fff;">Tạo Key Proxy Mới</div><div style="font-size:12px; color:#889;">Auto Gen 15 ký tự</div></div>
                </div>
            </div>
            ...
            <div class="select-btn" onclick="navTo('screen-admin-setup-proxy')">
                <div class="select-btn-left d-flex align-items-center gap-3">
                    ...
                    <div><div style="font-size:15px; font-weight:800; color:#fff;">Thiết Lập Proxy</div><div style="font-size:12px; color:#889;">Cấp Tên máy chủ, Random Cổng</div></div>
                </div>
            </div>
            ...
            <div class="select-btn" onclick="navTo('screen-admin-manage-keys'); loadAdminKeys();">
                <div class="select-btn-left d-flex align-items-center gap-3">
                    ...
                    <div><div style="font-size:15px; font-weight:800; color:#fff;">Quản Lý Kho Key</div><div style="font-size:12px; color:#889;">Ghim OLM, Thêm giờ</div></div>
                </div>
            </div>
        </div>
        ...
        <div id="screen-admin-keys" class="screen">
            ...
            <h3 style="margin-top:20px; color:#22c55e;"><i class="fas fa-key"></i> TẠO KEY PROXY</h3>
            <div class="card">
                ...
                <div style="display:flex; align-items:center; gap:10px; margin-bottom:20px;">
                    ...
                    <label for="k-vip" style="color:#ffcc00; font-weight:800;">Gắn thẻ VIP PRO</label>
                </div>
        ...
        <div id="screen-admin-setup-proxy" class="screen">
            <button class="btn-back" onclick="navTo('screen-admin-main')"><i class="fas fa-arrow-left"></i> Quay lại</button>
            <h3 style="margin-top:20px; color:#a855f7;"><i class="fas fa-server"></i> THIẾT LẬP PROXY (HOST & PORT)</h3>
            <div class="card">
                <p style="font-size:12px; color:#889;">Lưu ý: Bắt buộc Key phải được ghim tài khoản OLM trước thì mới cài được Proxy.</p>
                <input type="text" id="p-key" class="inp" placeholder="Nhập mã Key...">
                <input type="text" id="p-host" class="inp" placeholder="Tên máy chủ tùy chỉnh (VD: sv1.proxy.com)">
                <button class="btn-neon" style="background:#a855f7;" onclick="setupProxy()"><i class="fas fa-save"></i> LƯU THIẾT LẬP & RANDOM CỔNG</button>
            </div>
        </div>

        <div id="screen-admin-manage-keys" class="screen">
            <button class="btn-back" onclick="navTo('screen-admin-main')"><i class="fas fa-arrow-left"></i> Quay lại</button>
            <h3 style="margin-top:20px; color:#0099ff;"><i class="fas fa-database"></i> KHO LƯU TRỮ</h3>
        ...
        <script>
            ...
            function actKey() {
                ...
                api('/api/tg/activate_proxy', {tg_id:tgId, key:k}, r => {
                    if(r.status==='success') {
                        ...
                        Swal.fire('Thành Công', `Kích hoạt thành công cho OLM: ${r.olm}`, 'success');
                        ...
                        r.keys.forEach(k=>{
                            h+=`<div class="card">
                                <div style="display:flex;justify-content:space-between;">
                                    ...
                                    <span style="color:${k.vip?'#bd00ff':'#ffcc00'}; font-size:11px;">${k.vip?'VIP':'NOR'}</span>
                                </div>
                                <div style="color:#889; font-size:12px;">OLM: <span style="color:#fff;">${k.olm}</span></div>
                                <div style="color:#889; font-size:12px;">Host: <span style="color:#00ffcc;">${k.host}</span></div>
                                <div style="color:#889; font-size:12px;">Port: <span style="color:#00ffcc;">${k.port}</span></div>
                                <div style="color:#ff3366; font-size:12px;">Hạn: ${k.exp}</div>
                            </div>`;
                        });
                        document.getElementById('k_list').innerHTML = h || '<p>Trống.</p>';
                    }
                });
            }
            ...
            function createKeys() {
                ...
                api('/api/tg_admin/create_keys', {quantity: q, duration: d, unit: u, devices: dev, is_vip: v}, (res) => {
                    let kHtml = res.keys.map(k => `<div style="padding:10px; margin:5px 0; border:1px solid #333; color:#0ff;">${k}</div>`).join('');
                    Swal.fire({title: 'SẢN XUẤT THÀNH CÔNG', html: kHtml, background:'#1a1c26', color:'#fff'});
                });
            }
                    ...
                    if(r.status === 'success') {
                        ...
                        r.keys.forEach(k => {
                            h += `<div class="card admin-key-item" data-key="${k.key}">
                                <div style="display:flex; justify-content:space-between;">
                                    <strong style="color:#0ff;">${k.key}</strong>
                                    <span style="font-size:10px; color:${k.vip?'#bd00ff':'#ffcc00'};">${k.vip?'VIP':'NOR'}</span>
                                </div>
                                <div style="font-size:11px; color:#889;">
                                    OLM: <span style="color:#ffcc00">${k.olm || 'Chưa ghim'}</span> | Máy: <span style="color:#0ff">${k.devs}/${k.max_dev}</span><br>
                                    Proxy: ${k.host}:${k.port}<br>
                                    Hạn: ${k.exp}
                                </div>
            ...
            function bindOlmAdmin(k, old) {
                Swal.fire({
                    title: 'GHIM OLM', input: 'text', inputValue: old,
                    showCancelButton: true, background: '#1a1c26', color: '#fff'
                }).then((r) => {
