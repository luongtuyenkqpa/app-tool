...
.uptime{font-size:10px;color:var(--muted)}
.btn-logout{color:var(--danger);border:1px solid var(--danger);padding:5px 14px;border-radius:6px;font-size:11px;text-decoration:none}
.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:16px}
.stat{background:var(--card);border-radius:10px;padding:14px;text-align:center;border-top:3px solid}
.stat.s1{border-color:var(--success)}.stat.s2{border-color:var(--danger)}.stat.s3{border-color:var(--warn)}
.stat-n{font-size:28px;font-weight:900;margin-bottom:2px}
.stat.s1 .stat-n{color:var(--success)}.stat.s2 .stat-n{color:var(--danger)}.stat.s3 .stat-n{color:var(--warn)}
...
.inp{background:#0f172a;color:var(--text);border:1px solid var(--border);border-radius:6px;padding:9px 12px;font-size:13px;outline:none}
...
.btn{border:none;border-radius:6px;padding:9px 14px;font-size:12px;font-weight:700;cursor:pointer;text-transform:uppercase;letter-spacing:.5px;white-space:nowrap}
...
.badge{display:inline-block;padding:3px 8px;border-radius:4px;font-size:9px;font-weight:700;border:1px solid}
...
.log-ok{color:var(--success)}.log-block{color:var(--danger)}.log-act{color:var(--warn)}
.ef{display:inline-flex;gap:4px;align-items:center}
</style>
</head>
<body>
<div class="topbar">
...
<div class="card">
  ...
  <table>
    <thead><tr><th>ID Game</th><th>Hết Hạn</th><th>Còn Lại</th><th>Trạng Thái</th><th>Gia Hạn</th><th>Xóa</th></tr></thead>
...
.card{background:#111827;border:1px solid #1e3a5f;border-radius:14px;padding:32px;width:320px}
...
input{width:100%;background:#0f172a;color:#e2e8f0;border:1px solid #1e3a5f;border-radius:8px;padding:12px;font-size:14px;margin-bottom:14px;outline:none}
input:focus{border-color:#00d4ff}
button{width:100%;background:linear-gradient(135deg,#0066cc,#00d4ff);color:#fff;border:none;padding:13px;border-radius:8px;font-weight:700;font-size:13px;text-transform:uppercase;cursor:pointer}
...
<body>
<div class="card">
<h2>🛡 Admin</h2><p>FF MAX Auth System</p>
<form method="POST">
...
def render_admin():
    now = time.time()
    verified = GLOBAL_DB.get("verified", {})
    blocked_db = GLOBAL_DB.get("blocked", {})
    ...
    ud, ur = divmod(up, 86400); uh, ur = divmod(ur, 3600); um = ur // 60
    uptime_str = f"{ud}n {uh}h {um}p" if ud else (f"{uh}h {um}p" if uh else f"{um}p")
    ...
    for uid, exp in sorted(verified.items(), key=lambda x: x[1], reverse=True):
        ...
        rem_color = "#00ff88" if is_ok else "#ff4757"
        vrows += f"""<tr>
<td class="c-id">{uid}</td>
<td class="c-muted">{exp_str}</td>
<td><span style="color:{rem_color}">{rem}</span></td>
<td>{badge}</td>
<td>
  <form action="/admin/extend" method="GET" class="ef">
    <input type="hidden" name="id" value="{uid}">
    <input type="number" name="val" value="1" min="1" class="inp inp-sm" style="width:48px;padding:4px 6px;font-size:11px">
    <select name="unit" class="inp-unit" style="padding:4px 6px;font-size:11px">
      <option value="minutes">P</option>
      <option value="hours" selected>H</option>
      <option value="days">N</option>
      <option value="months">TH</option>
    </select>
    <button type="submit" class="btn btn-success btn-sm">+</button>
  </form>
</td>
<td><a href="/admin/delete?id={uid}" class="btn btn-danger btn-sm" onclick="return confirm('Xóa ID {uid}?')">XÓA</a></td>
</tr>"""
...
<td class="actions">
  <a href="/admin/add?id={bid}&val=30&unit=minutes" class="btn btn-sm" style="border:1px solid #64748b;color:#64748b;text-decoration:none">30P</a>
  ...
  <a href="/admin/add?id={bid}&val=1&unit=months" class="btn btn-sm" style="text-decoration:none;background:linear-gradient(135deg,#0066cc,#00d4ff);color:#fff;border:none">1TH</a>
</td>
</tr>"""
...
def admin_extend():
    ...
    if uid:
        ...
        add_log(uid, f"⏰ Gia hạn +{val}{unit[0]}")
    return redirect('/admin')
...
def make_fake():
    r = dict(_FAKE_BASE)
    r["data"] = dict(_FAKE_BASE["data"])
    r["data"]["server_time"] = int(time.time())
...
@app.route('/warmup')
def ping():
    return json.dumps({"status":"ok","uptime":int(time.time()-START_TIME)}), 200, {'Content-Type':'application/json'}
...
def game_handler(path=''):
    ...
    if uid:
        if check_verified(uid):
            ...
            r = _FAKE_BASE.copy()
            r["data"] = dict(_FAKE_BASE["data"])
            r.update({"status":"verified","code":0,"remaining_seconds":get_remaining(uid)})
            r["data"]["server_time"] = int(time.time())
            return game_resp(json.dumps(r, ensure_ascii=False), 200)
        else:
            add_blocked(uid)
    ...
    if path.lower().endswith(('.txt','.xml')):
        return game_resp("OK", 200, "text/plain")
    return game_resp(make_fake(), 200)

# =====================================================
# KEEP ALIVE - Tự ping mỗi 4 phút để không bị ngủ
# =====================================================
def keep_alive():
